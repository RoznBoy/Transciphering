# Rijndael S-Box
s_box = [
        
    ]

def key_expansion(key):
    """Perform AES key expansion."""
    # Round constants
    rcon = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d]

    # Number of words in the key
    words = len(key) // 4
    print('number of words :', words)
    # Number of rounds based on key size
    if words == 4:
        rounds = 10
    elif words == 6:
        rounds = 12
    elif words == 8:
        rounds = 14
    print('number of round : ', rounds)
    # Key schedule
    round_key = [0] * (4 * (rounds + 1))
    for i in range(words):
        round_key[i] = (key[4*i], key[4*i + 1], key[4*i + 2], key[4*i + 3])

    for i in range(words, 4 * (rounds + 1)):
        temp = round_key[i-1]
        if i % words == 0:
            temp = (temp[1], temp[2], temp[3], temp[0])
            temp = (s_box[temp[0]], s_box[temp[1]], s_box[temp[2]], s_box[temp[3]])
            temp = (temp[0] ^ rcon[i//words], temp[1], temp[2], temp[3])
        elif words > 6 and i % words == 4: # key size = 256
            temp = (s_box[temp[0]], s_box[temp[1]], s_box[temp[2]], s_box[temp[3]])
        round_key[i] = (round_key[i - words][0] ^ temp[0], round_key[i - words][1] ^ temp[1], round_key[i - words][2] ^ temp[2], round_key[i - words][3] ^ temp[3])

    return round_key

def print_state(state):
    """
    Helper function to print the state matrix.
    """
    for row in state:
        print([hex(byte) for byte in row])
    print()

# 테스트
plaintext = b"HelloAES12345678" #문자열 리터럴 앞에 b를 붙이면 해당 문자열이 바이트 문자열
key = b"secretkey1234567"

round_keys = key_expansion(key)

print_state(round_keys)
# 결과 출력
for i, key in enumerate(round_keys):
    print(f"Round Key {i}: {key}")

'''
print('===== Rcon test =====')
def generate_rcon(key):

    key_length = len(key)

    rounds = (len(key) // 4) + 6

    rcon = [0] * (rounds + 1)
    rcon[1] = 1  # Rcon[0]은 사용되지 않으므로 무시

    for i in range(2, rounds + 1):
        rcon[i] = (rcon[i-1] << 1) ^ (0x11b if rcon[i-1] & 0x80 else 0)

        # 키 길이가 32바이트 (AES-256)인 경우에만 추가적인 조건 적용
        if key_length == 32 and i % 8 == 0:
            rcon[i] ^= (rcon[i] >> 8) & 0xFF
            rcon[i] ^= (rcon[i] >> 16) & 0xFF
            rcon[i] ^= (rcon[i] >> 24) & 0xFF

    return rcon

rcon = generate_rcon(key)
print('rcon :', [hex(i) for i in rcon])
'''
